...
class MenuList extends Component{
        ...
        return (
            ...
            <div id={"menuList"} className={"container mt-5 pb-5 text-center"}>
                ...
                <div className={"row mt-5 mb-5"}>
                    {this.props.mealCategories.map((term)=>{
                        return(
                            <Link className={"btn col mealCategory-"+term.id} onClick={()=>this.props.onFilterByCategory(term.id)}
                                  to={`/menu/mealCategory/${term.id}`}  >
                                <span>{term.name}</span>
                            </Link>
                        )
                    })}
    ...
    handlePageClick=  /* istanbul ignore next */ (data)=>{
        ...
        return this.props.meals.map((term)=>{
            return (
                <div className={"col-md-4"}>
                    <MenuItem meal={term} onDelete={this.props.onDeleteMeal} onMealClick={this.props.onMealClick} onAddItemToOrder={this.props.onAddItemToOrder} />
                </div>
            )
